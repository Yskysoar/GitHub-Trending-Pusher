...
| 厂家标识       | 厂家名称   | Base URL                                          | 说明                |
| ---------- | ------ | ------------------------------------------------- | ----------------- |
| openai     | OpenAI | `https://api.openai.com/v1`                       | OpenAI官方          |
| volcengine | 火山方舟   | `https://ark.cn-beijing.volces.com/api/v3`        | 字节跳动火山方舟          |
| deepseek   | DeepSeek | `https://api.deepseek.com/v1`                     | DeepSeek官方        |
| zhipuai    | 智谱AI   | `https://open.bigmodel.cn/api/paas/v4`            | 智谱AI官方            |
| moonshot   | 月之暗面   | `https://api.moonshot.cn/v1`                      | Kimi大模型           |
| siliconflow | 硅基流动   | `https://api.siliconflow.cn/v1`                   | 硅基流动              |
| dashscope  | 阿里云百炼  | `https://dashscope.aliyuncs.com/compatible-mode/v1` | 阿里云百炼平台           |
| custom     | 自定义    | 用户自行填写                                            | 支持任意OpenAI兼容接口    |
...
| 字体   | 系统默认字体，中文使用微软雅黑               |
| 主色调  | `#1F6FEB`（GitHub蓝）            |
| 强调色  | `#238636`（成功绿）、`#DA3633`（警告红） |
| 圆角   | 组件圆角统一8px                     |
| 间距   | 基础间距8px，倍数递增（8/16/24/32）      |
| 动画   | 页面切换无动画，数据加载使用进度条             |
| 窗口大小 | 默认 960×640，最小 800×500         |
...
| `VOLCENGINE_API_KEY` | 否  | 空      | LLM API Key，优先级低于配置文件和.env（兼容旧版）              |
...
> **环境变量优先级**：系统环境变量 < 项目`.env`文件 < `app_config.json`。GUI保存设置时，敏感密钥同时写入`.env`文件和`app_config.json`。
| `APP_DATA_DIR`       | 否  | 程序所在目录 | 应用数据存储目录                               |
...
| `llm`        | `providers`              | object    | 预设代理厂家列表，包含厂家标识、名称和base_url                                                                                                                                 |
