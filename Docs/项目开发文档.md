...
| LLM集成 | openai | 1.0+ | 通过OpenAI兼容协议调用火山方舟GLM-4.7，一套代码兼容多家大模型服务商 |
...
| 火山方舟 Coding Plan | 字节跳动 | 提供GLM-4.7大模型API服务 | OpenAI兼容协议，接入简单，Coding Plan性价比高 |
...
3. 需要用户填写的配置项：
   ...
   - `llm.api_key`：火山方舟Coding Plan API Key
...
3. 创建大模型配置区域（API服务商、Base URL、API Key+测试连接、模型、Temperature）
