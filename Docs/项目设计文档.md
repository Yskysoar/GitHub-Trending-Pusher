...
| LLM集成      | openai         | 1.0+  | 通过OpenAI兼容协议调用火山方舟GLM-4.7                               |
...
| 火山方舟 Coding Plan | 字节跳动   | 提供GLM-4.7大模型API服务 |
...
本项目通过**OpenAI兼容协议**调用火山方舟Coding Plan的GLM-4.7模型。

#### 4.2.1 接口配置

| 配置项      | 值                                          | 说明                   |
| -------- | ------------------------------------------ | -------------------- |
| Base URL | `https://ark.cn-beijing.volces.com/api/v3` | 火山方舟OpenAI兼容接口地址     |
| API Key  | `sk-sp-xxx`                                | Coding Plan专用API Key |
| 模型标识     | `GLM-4.7`                                  | 智谱AI开源旗舰编程模型         |

> **注意**：火山方舟Coding Plan的OpenAI兼容接口中，`model`参数可直接使用模型名称（如`GLM-4.7`）。若后续接入非Coding Plan的火山方舟标准接口，则需使用Endpoint ID（格式：`ep-xxxxxxxx`）替代模型名称。

#### 4.2.2 Chat Completions 接口
...
| `VOLCENGINE_API_KEY` | 否  | 空      | 火山方舟API Key，优先级高于配置文件                  |
...
| `llm`        | `provider`               | string    | LLM服务商标识                                                                                                                                                     |
| `llm`        | `base_url`               | string    | API地址                                                                                                                                                        |
| `llm`        | `api_key`                | string    | API密钥                                                                                                                                                        |
| `llm`        | `model`                  | string    | 模型标识                                                                                                                                                         |
